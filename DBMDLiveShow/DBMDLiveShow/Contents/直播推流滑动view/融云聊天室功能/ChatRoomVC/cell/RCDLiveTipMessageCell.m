...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.tipMessageLabel = [[RCDLiveAttributedLabel alloc]init];
...
- (void)setDataModel:(RCDLiveMessageModel *)model {
    ...
    if ([content isMemberOfClass:[RCInformationNotificationMessage class]]) {
        ...
        self.tipMessageLabel.delegate=nil;
        ...
        NSString*str=notification.titleStr;
//        if (!str) {
//            return;
//        }
        //139 126 243
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:str];
        [attributedString addAttribute:NSForegroundColorAttributeName value:DBColor(139, 126, 243) range:[str rangeOfString:str]];
        self.tipMessageLabel.text=str;
        self.tipMessageLabel.delegate=nil;
        self.tipMessageLabel.attributedText = attributedString.copy;
...
{
     RCMessageContent *contentt = self.model.content;
    if ([contentt isMemberOfClass:[RCTextMessage class]]){
        RCTextMessage *notification = (RCTextMessage *)contentt;
        //        NSString *localizedMessage = [RCDLiveKitUtility formatMessage:notification];
        NSString *name=@"";
        if (notification.senderUserInfo) {
            //            name = [NSString stringWithFormat:@"%@:",notification.senderUserInfo.userId];
