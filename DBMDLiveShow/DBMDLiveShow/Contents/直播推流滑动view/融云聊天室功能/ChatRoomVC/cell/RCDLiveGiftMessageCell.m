 ...
 */
- (void)updateUI {
        self.giftImageView.frame=CGRectMake(6, 0, 20, 20);
    self.messageLabel.frame=CGRectMake(6+20+4, 0,KScreenWidth/2, 40);
...
//      }
        //        if ([_likeMessage.giftType isEqualToString:@"0"]) {
        //            self.messageLabel.text=[self.messageLabel.text stringByAppendingString:@"100椰子"];
        //        }else if ([_likeMessage.giftType isEqualToString:@"1"]){
        //            self.messageLabel.text=[self.messageLabel.text stringByAppendingString:@"贝壳"];
        //        }else if ([_likeMessage.giftType isEqualToString:@"2"]){
        //            self.messageLabel.text=[self.messageLabel.text stringByAppendingString:@"珊瑚"];
        //        }else if ([_likeMessage.giftType isEqualToString:@"3"]){
        //            self.messageLabel.text=[self.messageLabel.text stringByAppendingString:@"鲨鱼"];
        //        }else if ([_likeMessage.giftType isEqualToString:@"1"]){
        //            self.messageLabel.text=[self.messageLabel.text stringByAppendingString:@"游艇"];
        //        }
        
        if (_likeMessage.giftName) {
            self.messageLabel.text=[self.messageLabel.text stringByAppendingString:_likeMessage.giftName];

        }
        if (_likeMessage.giftImageStr) {
            [self.giftImageView sd_setImageWithURL:[NSURL URLWithString:_likeMessage.giftImageStr] placeholderImage:[UIImage imageNamed:@"placeholderPhoto"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
//    self.messageLabel.frame=CGRectMake(6+20+4, 0, __textSize.width, __textSize.height);
...
//collectionView 里面cell 的尺寸的
//得到这个item 的整体的宽和高
-(CGSize)getCollectionItemSize:(RCDLiveMessageModel*)model{
    RCDLiveGiftMessage *_likeMessage = (RCDLiveGiftMessage *)self.model.content;
    if (_likeMessage) {
        if(_likeMessage.senderUserInfo){
            if ([_likeMessage.senderUserInfo.userId isEqualToString:[RCDLive sharedRCDLive].currentUserInfo.userId]) {
...
//    self.messageContentView.frame = messageContentViewRect;
    ...
    return CGSizeMake(__textSize.width,  __textSize.height);
}
