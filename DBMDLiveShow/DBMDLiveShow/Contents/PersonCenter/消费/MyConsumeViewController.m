...
#define CELL0    @"MyConsumeTableViewCell"

@interface MyConsumeViewController ()<UITableViewDelegate,UITableViewDataSource,YJSegmentedControlDelegate>
...
@property(nonatomic,strong)NSString*payMoney;   //当前时间点消费的马币

@end

@implementation MyConsumeViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title=DBGetStringWithKeyFromTable(@"L消费记录", nil);
    [self addChooseView];
    [self addRefresh];
    [self.view addSubview:self.tableView];
    [self.tableView registerNib:[UINib nibWithNibName:@"MyConsumeTableViewCell" bundle:nil] forCellReuseIdentifier:CELL0];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MyConsumeTableViewCell*cell=[tableView dequeueReusableCellWithIdentifier:CELL0];
    cell.selectionStyle=NO;
    PresentRecordModel*model=self.allDatasModel[indexPath.row];
...
#warning 这里需要改变
    numberLabel.text=[NSString stringWithFormat:@"-%@",self.payMoney];
...
#warning 假数据
            ...
            //如果paymoney 不为nil
            self.payMoney=data[@"data"][@"payMoney"];
            
            for (NSDictionary*dict in data[@"data"][@"info"]) {
                PresentRecordModel*model=[PresentRecordModel yy_modelWithDictionary:dict];
...
//假数据
-(NSArray*)falueDatas{
    NSDictionary*dict=@{@"monthDay":@"3.30",@"hourMin":@"12:30",@"PayForAnchorPhoto":@"http://api.zhiboquan.net/Public/Upload/20170329/14907993835584.png",@"PayForAnchorNick":@"冯提莫",@"giftName":@"鲨鱼",@"giftMB":@"100.00"};
